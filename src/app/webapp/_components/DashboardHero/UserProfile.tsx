...
export function UserProfile({ user }: UserProfileProps) {
	const levelProgress = (user.exp % 100) / 100;
	const currentLevel = Math.floor(user.exp / 100);
		...
		>
			...
			<div className="flex-1 space-y-2">
				...
				<div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
					<span>最高ストリーク: {user.streak.best}日</span>
					<span>次のレベルまで: {100 - Math.floor(levelProgress * 100)}%</span>
				</div>
			</div>
		</motion.div>
	);
