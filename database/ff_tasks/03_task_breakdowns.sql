...
create table if not exists ff_tasks.task_breakdown_results (
    ...
    skill_category_id uuid references ff_skills.skill_categories(id),
...
create policy "システム管理者はタスク分解履歴を参照可能"
    on ff_tasks.task_breakdown_history
...
create policy "システム管理者はタスク分解履歴を削除可能"
    on ff_tasks.task_breakdown_history
    for delete using (ff_users.is_system_admin(auth.uid()));
...
create policy "システム管理者はタスク分解結果を参照可能"
    on ff_tasks.task_breakdown_results
    for select using (ff_users.is_system_admin(auth.uid())); 