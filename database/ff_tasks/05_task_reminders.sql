...
begin
    ...
    -- 通知の作成
    v_notification_id := ff_notifications.create_notification(
        v_reminder.task_owner_id,
        'task_reminder',
        case v_reminder.reminder_type
            when 'deadline' then 'task_deadline_reminder'
            when 'start_time' then 'task_start_reminder'
            else 'task_general_reminder'
        end,
        v_template_data
    );
