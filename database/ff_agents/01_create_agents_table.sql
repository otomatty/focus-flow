create schema if not exists ff_agents;

-- エージェントの基本情報を管理するテーブル
create table if not exists ff_agents.agents (
...
-- キャラクター設定の例：
COMMENT ON COLUMN ff_agents.agents.character IS '
{
    "age": "28歳",
    "gender": "女性",
    "traits": ["優美", "繊細", "思慮深い"],
...
-- インデックスの作成
CREATE INDEX idx_agents_is_default ON ff_agents.agents(is_default);
CREATE INDEX idx_agents_name ON ff_agents.agents(name);
...
CREATE TRIGGER update_agents_updated_at
    BEFORE UPDATE ON ff_agents.agents
    FOR EACH ROW
    EXECUTE FUNCTION update_agents_updated_at_column();

-- デフォルトエージェントの作成
INSERT INTO ff_agents.agents (
    id,
    name,
    avatar_url,
    personality,
