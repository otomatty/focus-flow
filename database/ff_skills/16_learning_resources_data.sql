-- Reactスキルの学習リソース
with react_skill as (
  select id from ff_skills.skill_details where slug = 'react'
)
insert into ff_skills.learning_resources (
  skill_id,
  type,
  name,
  provider,
...
with nodejs_skill as (
  select id from ff_skills.skill_details where slug = 'nodejs'
)
insert into ff_skills.learning_resources (
  skill_id,
  type,
...
with typescript_skill as (
  select id from ff_skills.skill_details where slug = 'typescript'
)
insert into ff_skills.learning_resources (
  skill_id,
  type,
...
with tailwind_skill as (
  select id from ff_skills.skill_details where slug = 'tailwind-css'
)
insert into ff_skills.learning_resources (
  skill_id,
  type,
...
with nextjs_skill as (
  select id from ff_skills.skill_details where slug = 'nextjs'
)
insert into ff_skills.learning_resources (
  skill_id,
  type,
