-- スキル評価指標テーブル
create table if not exists skill_evaluation_metrics (
...
comment on table skill_evaluation_metrics is 'スキル評価の指標を管理するテーブル';

-- 初期データ: 評価指標
insert into skill_evaluation_metrics (name, slug, description, category, evaluation_type, weight) values
-- 成果の質的評価 (quality)
...
create table if not exists user_skill_evaluations (
  id uuid primary key default uuid_generate_v4(),
  user_skill_id uuid references user_skills(id) not null,
  metric_id uuid references skill_evaluation_metrics(id) not null,
...
comment on table user_skill_evaluations is 'ユーザーのスキル評価データを管理するテーブル';

-- ランク昇進の評価基準テーブル
create table if not exists rank_evaluation_criteria (
  id uuid primary key default uuid_generate_v4(),
...
comment on table rank_evaluation_criteria is 'ランク昇進の評価基準を管理するテーブル';
...
comment on table evaluation_history is 'スキル評価の履歴を管理するテーブル'; 