-- シーズン管理用のスキーマ作成
create schema if not exists ff_seasons;

-- シーズンマスターテーブル
create table if not exists ff_seasons.seasons (
...
-- ユーザーシーズン進捗テーブル
create table if not exists ff_seasons.user_progress (
    id uuid primary key default uuid_generate_v4(),
    user_id uuid references auth.users(id) not null,
    season_id uuid references ff_seasons.seasons(id) not null,
...
-- ユーザーシーズン履歴テーブル
create table if not exists ff_seasons.user_history (
    id uuid primary key default uuid_generate_v4(),
    user_id uuid references auth.users(id) not null,
    season_id uuid references ff_seasons.seasons(id) not null,
...
-- シーズン監査ログテーブル
create table if not exists ff_seasons.audit_logs (
    id uuid primary key default uuid_generate_v4(),
    season_id uuid references ff_seasons.seasons(id) not null,
...
-- シーズンランク設定テーブル
create table if not exists ff_seasons.rank_settings (
    id uuid primary key default uuid_generate_v4(),
    season_id uuid references ff_seasons.seasons(id) not null,
...
-- テーブルコメント
comment on table ff_seasons.seasons is 'シーズンの基本情報を管理するテーブル';
comment on table ff_seasons.user_progress is 'ユーザーごとのシーズン進捗を管理するテーブル';
comment on table ff_seasons.user_history is 'ユーザーごとのシーズン履歴を保存するテーブル';
comment on table ff_seasons.audit_logs is 'シーズン関連の操作履歴を記録するテーブル';
comment on table ff_seasons.rank_settings is 'シーズンごとのランク設定を管理するテーブル';
comment on column ff_seasons.rank_settings.required_points is 'ランクに必要な最小ポイント';
comment on column ff_seasons.rank_settings.focus_time_requirement is '累計の必要集中時間';
comment on column ff_seasons.rank_settings.task_completion_requirement is '累計の必要タスク完了数';
comment on column ff_seasons.rank_settings.daily_focus_requirement is '1日あたりの必要集中時間';
comment on column ff_seasons.rank_settings.weekly_focus_requirement is '週あたりの必要集中時間';
comment on column ff_seasons.rank_settings.maintenance_requirements is 'ランク維持に必要な要件（JSON形式）';
comment on column ff_seasons.rank_settings.demotion_threshold is 'このポイント以下になると降格';
comment on column ff_seasons.rank_settings.promotion_threshold is 'このポイント以上で昇格可能';
comment on column ff_seasons.rank_settings.rewards is 'ランク報酬設定（JSON形式）';

-- インデックス
create index if not exists idx_seasons_status on ff_seasons.seasons(status);
create index if not exists idx_user_progress_user_season on ff_seasons.user_progress(user_id, season_id);
create index if not exists idx_user_history_user_season on ff_seasons.user_history(user_id, season_id);
create index if not exists idx_audit_logs_season on ff_seasons.audit_logs(season_id);
create index if not exists idx_audit_logs_user on ff_seasons.audit_logs(user_id);
create index if not exists idx_audit_logs_created_at on ff_seasons.audit_logs(created_at);
create index if not exists idx_rank_settings_season on ff_seasons.rank_settings(season_id);

-- 更新日時自動更新のトリガー関数
create or replace function ff_seasons.update_updated_at_column()
returns trigger as $$
begin
    new.updated_at = now();
    return new;
...
create trigger update_seasons_updated_at
    before update on ff_seasons.seasons
    for each row
    execute function ff_seasons.update_updated_at_column();

create trigger update_user_progress_updated_at
    before update on ff_seasons.user_progress
    for each row
    execute function ff_seasons.update_updated_at_column();

-- シーズン終了時のユーザー履歴保存関数
create or replace function ff_seasons.archive_user_season_history(
    p_season_id uuid,
    p_user_id uuid
)
returns void as $$
...
begin
    ...
    -- 履歴データを保存
    insert into ff_seasons.user_history (
        user_id,
        season_id,
        final_rank,
        total_points,
    ...
    values (
        ...
        case when v_progress.rewards_claimed then
            (select rewards from ff_seasons.seasons where id = p_season_id)
        else '[]'::jsonb end,
        jsonb_build_object(
            'daily_averages', jsonb_build_object(
                'focus_time', extract(epoch from v_progress.total_focus_time) / 
                    extract(days from (select end_date - start_date from ff_seasons.seasons where id = p_season_id)),
...
-- 監査ログ記録関数
create or replace function ff_seasons.log_action(
    p_season_id uuid,
    p_user_id uuid,
    p_action_type text,
...
-- シーズンステータス更新時のトリガー関数
create or replace function ff_seasons.on_season_status_change()
returns trigger as $$
begin
...
create trigger season_status_change_trigger
    after update of status on ff_seasons.seasons
    for each row
    execute function ff_seasons.on_season_status_change();

-- ユーザー進捗更新時のトリガー関数
create or replace function ff_seasons.on_user_progress_change()
returns trigger as $$
begin
...
create trigger user_progress_change_trigger
    after update of current_rank on ff_seasons.user_progress
    for each row
    execute function ff_seasons.on_user_progress_change();

-- デフォルトのランク設定を挿入する関数
create or replace function ff_seasons.create_default_rank_settings(p_season_id uuid)
returns void as $$
begin
    -- Bronze
    insert into ff_seasons.rank_settings (
...
create or replace function ff_seasons.on_season_created()
returns trigger as $$
begin
    perform ff_seasons.create_default_rank_settings(NEW.id);
    return NEW;
end;
$$ language plpgsql;

create trigger season_created_trigger
    after insert on ff_seasons.seasons
    for each row
    execute function ff_seasons.on_season_created(); 