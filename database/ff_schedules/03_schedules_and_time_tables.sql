...
create type ff_schedules.visibility_type as enum ('private', 'public', 'followers');
...
-- タイムテーブル関連のテーブル
create table ff_schedules.templates (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users not null,
  name text not null,
  description text,
  color_id uuid references ff_schedules.color_palette,
  visibility ff_schedules.visibility_type not null default 'private',
...
-- テンプレートのいいね管理
create table ff_schedules.template_likes (
  id uuid primary key default gen_random_uuid(),
  template_id uuid references ff_schedules.templates on delete cascade not null,
  user_id uuid references auth.users not null,
  created_at timestamp with time zone default now(),
  unique(template_id, user_id)
);

create table ff_schedules.template_apply_days (
  id uuid primary key default gen_random_uuid(),
  template_id uuid references ff_schedules.templates on delete cascade not null,
  day_of_week integer not null check (day_of_week between 0 and 6),
  created_at timestamp with time zone default now(),
...
create table ff_schedules.template_time_slots (
  id uuid primary key default gen_random_uuid(),
  template_id uuid references ff_schedules.templates on delete cascade not null,
  title text not null,
  description text,
...
create trigger update_templates_updated_at
  before update on ff_schedules.templates
  for each row
  execute function ff_schedules.update_updated_at();
...
create trigger update_template_likes_count
  ...
  for each row
  execute function ff_schedules.update_template_likes_count();
...
create index idx_templates_user_id on ff_schedules.templates(user_id);
...
create index idx_templates_visibility on ff_schedules.templates(visibility);
create index idx_templates_is_featured on ff_schedules.templates(is_featured);
create index idx_templates_likes_count on ff_schedules.templates(likes_count);
create index idx_template_likes_user_id on ff_schedules.template_likes(user_id);
create index idx_template_likes_template_id on ff_schedules.template_likes(template_id);
...
alter table ff_schedules.templates enable row level security;
...
create policy "Users can view their own templates"
  on ff_schedules.templates for select
  using (auth.uid() = user_id);

create policy "Users can insert their own templates"
  on ff_schedules.templates for insert
  with check (auth.uid() = user_id);

create policy "Users can update their own templates"
  on ff_schedules.templates for update
  using (auth.uid() = user_id);

create policy "Users can delete their own templates"
  on ff_schedules.templates for delete
  using (auth.uid() = user_id);

-- テンプレートいいね用のポリシー
create policy "Users can view template likes"
  on ff_schedules.template_likes for select
  using (true);

create policy "Users can like templates they can view"
  on ff_schedules.template_likes for insert
  with check (
    auth.uid() = user_id
    and exists (
        select 1 from ff_schedules.templates
...
create policy "Users can unlike their likes"
  on ff_schedules.template_likes for delete
  using (auth.uid() = user_id);
